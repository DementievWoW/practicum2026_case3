-- Сгенерировано из data_model_sql/data_model.sql; CREATE DATABASE/\connect выкинуты
-- (compose создаёт БД через POSTGRES_DB и пользователя через POSTGRES_USER).
-- Финальный COMMIT добавлен, т.к. исходный дамп открывает BEGIN без COMMIT.
